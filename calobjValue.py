...
#Decode function of whole population(针对整个种群的解码函数)
...
#Function to calculate the start time                        计算开始时间的函数
#Pop: The population after the decoding (经过解码之后的种群)
#Chrom_length: gene length    (基因长度)
#Timelist    : working time   (供电时间表)
def caltimeValue(Pop_decoded,Chrom_length,Timelist):
    Variable = 0 
    Temp_pop_start = []                            #recording the starting time of appliances用来记录电器开始工作的时间
    Temp_pop_end = []                              #recording the ending time of appliances用来记录电器结束工作的时间
    for i in range(len(Timelist)):
        for j in range(len(Timelist[i])//2):
            ...
            if Starttime < Endtime:
                Period = Endtime - Starttime       #The working period of every appliance计算每个用电器的使用时长
            else:
            ...
            for k in range (len(Pop_decoded[Variable])):        #用电器仅使用一次的情况
                ...
                if temp > 24.0:
                    temp = temp - 24.0    #Adjust the range of time to ensure it is located in 0-24 hrs调整时间范围，确保时间段落入0——24小时中
                elif temp < 0:
                    temp = -temp 
...
def calobjValue(Pop, Chrom_length, Timelist, Species_size):
    Pop_decoded = []                     
    Pop_decoded = decodechrom(Pop, Chrom_length)   #The population after decoding (经过解码的种群)
    Temp_pop_start,Temp_pop_end = caltimeValue(Pop_decoded, Chrom_length, Timelist)
    return Temp_pop_start, Temp_pop_end


#calculate the user's cost    (计算用户电费的程序)
def calculateCost(User1):
    Peak_starttime = 6.0       #Peak period starting time   峰电开始时间
    Peak_endtime = 22.0        #Peak period ending time   峰电结束时间
    Peak_period = 16.0         #Time period of peak period   峰电持续时间(Peak_endtime - Peak_starttime)
    
    Valley = 0.6               #valley price   谷电电价
    Peak = 1.2                 #Peak price   峰电电价    
    
    Cost = 0                   #Total cost总电费
    Time_slots = 0             #Total time slots of all appliances   总使用电器时间段数目
    Timelist = []              #Start timelist   开始时间表
    Individual_cost = []       #cost list of Single item   单项电费表
    
    for i in range((len(User1)//2)):    
        Temp = User1[2*i+1]        #Storage the list of appliance' working period   存储电器的使用时间的列表
        Power = User1[2*i][0]         #Storage the current appliance power value   存储当前使用电器功率值
        Timelist.append(User1[2*i+1][0])
        #print(Temp)
        #Considering all circumstances, there are totally 12 ways to calculate the electricity cost  s考虑所有的情况，一共有12种电费计价方式
        j = 0
        for j in range(len(Temp[0])//2):
            ...
            #Case 4, Starting time is larger than the end of end time情况四，开始时间大于峰值结束时间
...
#Get the timelist, calculate the cost of every condition得到时间表之后，计算每一种情况电费的程序
def calcostValue(Temp_pop_start,Temp_pop_end,User):
    Cost_list = []             #electricity price list电费列表
    for i in range(len(Temp_pop_start)):
        Cost_list.append([])

    for k in range(len(Temp_pop_start[0])):
        Variable = 0               #The working time slots of appliance电器使用时间段数
